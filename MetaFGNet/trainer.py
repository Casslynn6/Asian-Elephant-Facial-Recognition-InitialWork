...
def train(train_loader_source, train_loader_source_batch, train_loader_target, train_loader_target_batch, model_source, model_target, criterion, optimizer, epoch, args, meta_train_lr):
    ...
    top1_target.update(prec1, input_target_mtrain.size(0))
    top5_target.update(prec5, input_target_mtrain.size(0))
    ...
    if train_loader_source:
        ...
        top1_source.update(prec1, input_source.size(0))
        top5_source.update(prec5, input_source.size(0))
    ...
    if epoch % args.record_freq == 0:
        log = open(os.path.join(args.log_folder, 'log.txt'), 'a')
        log.write("\n")
...
def validate(val_loader_source, val_loader_target, model_source, model_target, criterion, epoch, args):
    ...
    for i, (input_target, target_target) in enumerate(val_loader_target):  # the iterarion in the source dataset.
        ...
        top1_target.update(prec1, input_target.size(0))
        top5_target.update(prec5, input_target.size(0))
        batch_time.update(time.time() - end)
        end = time.time()
        if i % args.print_freq == 0:
            print('Te T: [{0}][{1}/{2}]\t'
                  ...
                  'LT {target_loss.val:.4f} ({target_loss.avg:.4f})'.format(
                   ...
                   target_top1=top1_target, target_top5=top5_target, target_loss=losses_target))
    if val_loader_source:
        print(' * Source Dataset   Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f}'
...
def test(test_loader, model_source, model_target, criterion, epoch,args):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses_source = AverageMeter()
    top1_source = AverageMeter()
    ...
    for i, (test_images, test_labels) in enumerate(test_loader):
        ...
        test_labels = test_labels.cuda(async = True)
        output_target = model_target(test_images)

        # Compute loss
        loss_target = criterion(output_target, test_labels)
        prec1, prec5 = accuracy(output_target.data, test_labels, topk=(1, 5))

        losses_target.update(loss_target.data[0], test_images.size(0))
        top1_target.update(prec1, test_images.size(0))
        top5_target.update(prec5, test_images.size(0))
    ...
    print(' * Test Dataset ({}) Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f}'
          .format(args.testdataset, top1=top1_target, top5=top5_target))
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    lr = args.learning_rate * (args.gamma ** exp)
    lr_pretrain = args.learning_rate * (args.gamma ** exp_pretrain)
